...
MMDB 文件通常较大，且可能受各数据源许可限制。建议不要直接提交到 GitHub 仓库，可在部署环境中单独放置，或通过环境变量指定路径。
...
- `.env`
- API Key、Token、TURN 凭据等密钥
- 未确认授权或体积过大的 MMDB 数据库文件
- 本地构建产物和编辑器缓存

## 注意事项

- `.env` 已被 `.gitignore` 忽略，不要提交 API Key 或 TURN 凭据。
